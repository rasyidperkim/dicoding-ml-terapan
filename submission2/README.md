...
Dari Gambar 1, beberapa informasi yang didapatkan antara lain :
...
Berdasarkan hasil perhitungan manual sebanyak 2 kali uji coba, 5 *item* benar-benar relevan dari 5 *item* yang direkomendasikan kepada pengguna dan tidak ada *item* yang tidak relevan dengan pengguna. Maka nilai *precision* sistem rekomendasi tersebut adalah 5/(5+0) = 1.0. Nilai *precision* 1.0 menunjukkan bahwa sistem rekomendasi tersebut sangat presisi dalam merekomendasikan item yang relevan dengan pengguna, baik dengan *Cosine Similarity* maupun *Jaccard Similarity*, sehingga dapat dikatakan model *Content-based Filtering* pada proyek ini dalam merekomendasikan sudah bagus.
